...
def detect_faces(filepath, classifier,file_name, id_pertemuan, scaleFactor=1.1, minNeighbors=3):
    ...
    for i,(top, right, bottom, left) in enumerate(faces):
        ...
        rect = {'color': '#00ff00', 'top': int(top), 'bottom': int(bottom),
                'left': int(left), 'right': int(right), 'NRP' : '','Name':'','Id_Photo': str(last_photo_id) ,'path': output_path, }
        faces_to_json.append(rect)
    # save detected face areas as json file
    message = ""
    if(len(faces_to_json)== 0):
    	message ="Tidak ada muka terdeteksi"
    else:
    	message ="Data Berhasil Disimpan"
    	
    output_json = {'data': faces_to_json, 'message': message}
    with open( local_folderpath + name + '.json', 'w') as outfile:
        json.dump(obj=output_json, fp=outfile, indent=4)
    #with open('../public/' + filepath + name + '.log', 'w' ) as logfile :
        #json.dump(obj=sql_to_json,fp=logfile,indent=4)

    #create meta file
    #sql = "select  m.id_mk, m.nama_mk, c.nama_kelas from kelas c join mk m on (c.id_mk = m.id_mk) join pertemuan p on (p.id_kelas = c.id_kelas) where p.Id_pertemuan=" + id_pertemuan
    #print(sql)
    #my_cursor.execute(sql)
