...
class UserKelasController extends BaseController
{
    ...
    foreach ($result as $r) {
        $r->kelas = $r->kelas;
        $tipe_kelas = ($r->kelas->tipe_kelas == 0)? " Teori" : " Praktikum ";
        $r->kelas->tipe_kelas =  $tipe_kelas;

        $r->kelas->mk = $r->kelas->mk->get();

        // $r->nama_kelas_total = $r->kelas->mk->nama_mk . " " + $tipe_kelas;
        // $r->user = $r->user;
    }
    // return $result;
    ...
    public function get_kelas_by_id_custom (Request $request) {
        ...
        foreach ($result as $r) {
            ...
            $tipe = ($r->kelas->tipe_kelas == 0)? " Teori" : " Praktikum ";
            $temp->nama_mk = $r->kelas->mk->nama_mk . $tipe;
